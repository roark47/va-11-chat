...
- 用户按频道输入密码登录
- 使用原生 WebSocket 实时聊天
- WebSocket 断线自动重连，网络恢复或手机回到前台时自动补齐历史消息
- 不同频道消息隔离
- 消息显示昵称和内容
- 其他用户发消息时可触发浏览器通知
...
- 不使用数据库、ORM 或组件库
- 兼容桌面和移动端浏览器的响应式布局
- 可安装为 PWA，并从手机桌面自动恢复上次登录的聊天室

## 手机桌面使用

用户第一次通过聊天室链接登录时，可以勾选“在此设备保留席位 30 天”。进入聊天室后，
点击 **Install app**；在 iPhone 上请使用系统分享菜单中的“添加到主屏幕”。此后从桌面图标
启动时，应用会验证设备会话并直接进入上次登录的聊天室，不需要再次寻找链接或输入密码。

设备会话保存在 `HttpOnly` Cookie 中，前端不会把明文密码写入 `localStorage`。点击
**Leave the seat** 会立即清除设备会话。

> 当前通知只在页面仍能运行时生效。手机系统完全挂起网页后如需继续收到通知，还需要部署
> 标准 Web Push 服务。
